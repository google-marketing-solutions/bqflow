...
try:
  ...
  raise ModuleNotFoundError('PLEASE RUN: python3 -m pip install smartsheet-python-sdk') from e

from bqflow.util.csv import column_header_sanitize
...
def get_rows(config, sheet=None, report=None, header=True, link=True, key='id'):
...
def smartsheet(config, log, task):
  if config.verbose:
    print('SMARTSHEET')

  if 'sheet' in task:
    link = task.get('link', True)
    rows = get_sheet_rows(config, task['token'], task['sheet'], link)
    schema = get_sheet_schema(task['token'], task['sheet'], link)

  elif 'report' in task:
    link = False
    rows = get_report_rows(config, task['token'], task['report'])
  ...
  if link: 
    task['results']['bigquery']['schema'].insert(0, {
      'name': 'rowPermalink',
      'type': 'STRING',
      'mode': 'NULLABLE'
    })
  ...
  if rows:
    return put_rows(config, task['auth'], task['results'], rows)
